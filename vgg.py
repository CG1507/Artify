...
#change for independent execution
vgg16_weights = "./weights/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5"
...
parser.add_argument("--num_iter", dest="num_iter", default=1000, type=int,
...
for i in range(len(feature_layers) - 1):
	...
	for j in range(nb_style_images):
		loss += (style_weights[j] / len(feature_layers)) * sl1[j]
...
for i in range(num_iter):
	...
	#if i % 50 == 0:
	#change for independent execution
	fname = '../results/gui/vgg/' + result_prefix + "_at_iteration_%d.png" % (i + 1)
	imsave(fname, img)
	end_time = time.time()
